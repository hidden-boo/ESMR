...
from user_env_emotion_only_ablation import UserEnv
from load_env_data import load_env_data
from train_q_learning_emotion_only import QLearningAgent
import numpy as np
...
if __name__ == "__main__":
    ...
    causal_path = "top_causal_parents.csv"
    lgbm_recs_path ="daily_recommendations.json"
    q_table_path = "q_table_emotion_only_no_causal_ablation.pkl"
    ...
    out_dir = "Phase-III\Ablations\Output_training"
    ...
    safe_json_dump(user_data_dict, os.path.join(out_dir, "user_data_dict_emotion_only_ablation.json"), "user_data_dict_emotion_only_ablation.json")
    safe_json_dump(rl_mode_days_dict, os.path.join(out_dir, "rl_mode_days_dict_emotion_only_ablation.json"), "rl_mode_days_dict_emotion_only_ablation.json")
    safe_json_dump(episode_logs_by_user, os.path.join(out_dir, "episode_logs_by_user_emotion_only_ablation.json"), "episode_logs_by_user_emotion_only_ablation.json")
    safe_json_dump(content_emotions_dict, os.path.join(out_dir, "content_emotions_dict_emotion_only_ablation.json"), "content_emotions_dict_emotion_only_ablation.json")
    safe_json_dump(video_metadata_dict, os.path.join(out_dir, "video_metadata_dict_emotion_only_ablation.json"), "video_metadata_dict_emotion_only_ablation.json")
