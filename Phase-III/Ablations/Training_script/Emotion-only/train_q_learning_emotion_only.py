...
class QLearningAgent:
    ...
    def train(self, epochs=10):
        ...
        for epoch in range(epochs):
            ...
            for uid in user_ids:
                ...
                while not user_env.done:
                    ...
                    if q_vals:
                        q_array = np.array(list(q_vals.values()))
                        probs = np.exp(q_array) / (np.sum(np.exp(q_array)) + 1e-6)
                        entropy = -np.sum(probs * np.log(probs + 1e-6))
                        if not np.isnan(entropy) and not np.isinf(entropy):
                            reward += -0.05 * entropy
                        else:
                            print(f"[WARN] Entropy became invalid. Skipping entropy penalty.")
                    #print(f"UID: {uid}, Day: {user_env.day}, Reward: {reward:.4f}")
        ...
        self.save_outputs(label="hybrid_rl_lightgbm_dynamic")
    ...
    def save_outputs(self, label="hybrid_rl_lightgbm_dynamic"):
        base_path = "Phase-III\Ablations\Output_training\Emotion-only"
...
if __name__ == "__main__":
    ...
    agent.save_outputs(label="emotion_only_no_causal_ablation") 
