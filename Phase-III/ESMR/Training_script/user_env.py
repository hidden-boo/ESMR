...
class UserEnv(gym.Env):
    ...
    def _has_sustained_negative_streak(self, threshold=3, intensity_threshold=0.2):
        negative_streak = 0
        for i in range(self.current_day - 1, self.current_day - threshold - 1, -1):
            if i < 0:
                break
            user_day_data = self.user_data[self.current_user][i]
            dominant_emotion = user_day_data.get("dominant_emotion", "neutral")
    ...
    def step(self, action):
        ...
        # ===== Causal RL Mode =====
        if use_rl:
            #print("[DEBUG] Calculating causal bonus:")
            for _, parent_list in self.causal_top_parents.items():
                for parent_feat, weight in parent_list:
                    if parent_feat in video_meta and isinstance(video_meta[parent_feat], (int, float)):
                        scaled = float(weight) * float(video_meta[parent_feat])
                        # Vulnerable state scaling
                        if current_emotion in self.vulnerable_states:
                            scaled *= 1.5
                        causal_bonus += scaled
            reward += causal_bonus
            #print(f"[DEBUG] Causal Bonus: {causal_bonus}")
            ...
            #print(f"[DEBUG] Threshold Bonus: {threshold_bonus}")

            # Positive transition bonus
            if prev_user_emotion in self.negative_emotions and current_emotion in self.positive_emotions:
                reward += 1.5
                #print(f"[DEBUG] Positive transition bonus")

            # Vulnerable emotion penalty
            if current_emotion in self.vulnerable_states:
                emotion_penalty -= 1.0
                reward += emotion_penalty
                #print(f"[DEBUG] Emotion Penalty: {emotion_penalty}")
